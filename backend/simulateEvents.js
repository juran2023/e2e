...
async function simulateEvents() {
  ...
  console.log(`created ${createdUsers.length} users: ${createdUsers}`)
  ...
  console.log(`created ${createdPosts.length} posts: ${createdPosts}`)
  ...
  const createdViews = await Promise.all(
    Array(simulatedViews)
      ...
      .map(async () => {
        ...
        const sessionEnd = sessionStart + Math.random() * 1000 * 60 * 5
